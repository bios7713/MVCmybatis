...
function checkQty(num, qty){
	if(qty > 1){
		location.href="goodsQtyDown?goodsNum="+num;
	}else{
		return false;
	}
}
function goPayment(){		
	location.href="goodsCartPayment"
}
</script>
</head>
<body>
...
<table align="center" width="600" border="1">
	...
	<tr align="center" bgcolor="orange">
		<td>${cart.goodsSeq }	</td>
		<td>
		<c:forTokens items="${cart.goodsImage }" delims="-" var="image" varStatus="ii">
			<c:if test="${ii.index == 0 }">
		       <img src = "../goodsView/update/${image}" width="30"/>
		   </c:if>
		</c:forTokens>
		</td>
		<td>${cart.goodsName }</td>
		<td>${cart.goodsPrice }</td>
		<td><a href="goodsQtyUp?goodsNum=${cart.goodsSeq }"> + </a> 
		 ${cart.qty } 
